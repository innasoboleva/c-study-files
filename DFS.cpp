...
queue<int> doDepthFirstSearch(int iOriginNode, vector<Node>& database)
{
    ...
    while(!toDo.empty())
    {
        ...
        if(!(database.at(ind).isVisited)) // if that node has not yet been visited
        {
            database.at(ind).isVisited = true; // mark the node as visited
            searchOrder.push(ind); // push node's index onto the result queue
            for(list<int>::reverse_iterator it = database.at(ind).neighbors.rbegin(); it != database.at(ind).neighbors.rend(); ++it)
            {
                if(!(database.at(*it).isVisited)) // if neighbor has not yet been visited
                {
                    toDo.push(*it); // push neighbor's index onto the "to-do" list
                }
            }
        }
    }
